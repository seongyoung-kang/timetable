	...
	<div class="header_wrap xe-clearfix">
		<?php if($__Context->layout_info->enable_intergration_search === 'Y'){ ?><div class="search_wrap">
			<div class="search_area">
				...
				<!-- /SEARCH -->
				<a href="#" class="btn_close" title="<?php echo $__Context->lang->cmd_UNI_TIME_search_close ?>" onclick="return false"><i class="xi-close"></i><span class="blind"><?php echo $__Context->lang->cmd_UNI_TIME_search_close ?></span></a>
			</div>
		</div><?php } ?>
...
$__Context->_logo_img = 's_logo.png';
						...
						<img src="<?php echo $__Context->layout_info->path ?>img/<?php echo $__Context->_logo_img ?>"<?php if($__Context->_onepage_header){ ?> data-logo="<?php echo $__Context->layout_info->path ?>img/logo.png"<?php } ?> alt="UNI_TIME" />
					<?php } ?>
				</a>
			</h1>
...
<!-- /TOP -->
<!-- Login widget -->
<?php if($__Context->layout_info->use_login_widget != 'N'){ ?><section class="login_widget"<?php if($__Context->XE_VALIDATOR_ID != 'layouts/UNI_TIME/layout/1' || !$__Context->XE_VALIDATOR_MESSAGE){ ?> style="display:none"<?php } ?>>
	...
	<div class="signin">
		...
		<div class="login-body">
			<form action="<?php echo getUrl('', 'act', 'procMemberLogin') ?>" method="post" autocomplete="off"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
				...
				<input type="hidden" name="xe_validator_id" value="layouts/UNI_TIME/layout/1" />
				<fieldset>
					...
					<?php if($__Context->XE_VALIDATOR_ID == 'layouts/UNI_TIME/layout/1' && $__Context->XE_VALIDATOR_MESSAGE){ ?><div class="control-group">
